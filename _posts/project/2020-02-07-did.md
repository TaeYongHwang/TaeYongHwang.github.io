...
> Claim : Credential subject의 ID 속성에 대한 정보가 Subject-Property-Value 방식으로 저장 (현재 non-normative)된다.
> 여러 개의 subject가 명시될 수 있다.
> proofs : VC에 대한 진위 여부 검증에 필요한 값이 포함된다. ( ECDSA, 생체인증 등 다양한 암호 기법이 사용될 수있다.) 검증인은
